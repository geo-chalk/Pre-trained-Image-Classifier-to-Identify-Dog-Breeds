...
def print_results(results_dic, results_stats_dic, model, 
    ...
    print("\n\n*** Results Summary for CNN Model Architecture",model.upper(), 
          "***")
    print("{:30}: {:1d}".format('Number of Images', results_stats_dic['n_images']))
    print("{:30}: {:1d}".format('Number of Dog Images', results_stats_dic['n_dogs_img']))
    print("{:30}: {:1d}".format('Number of "Not-a" Dog Images', results_stats_dic['n_notdogs_img']))

    print("\n*** Percentages ***")
    for key,value in results_stats_dic.items():
        if key[0] == 'p':
            print("{:30}: {:4.2f}".format("Percentage of " +" ".join(key.split("_")[1:]), value))
    
    #Misclassified Dogs if print_incorrect_dogs == True
